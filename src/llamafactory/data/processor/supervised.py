...
class SupervisedDatasetProcessor(DatasetProcessor):
    ...
    def _encode_data_example(
        ...
        messages = self.template.mm_plugin.process_messages(
            prompt + response, images, videos, audios, self.processor, **lipread_kwargs
        )
    ...
    def _encode_data_examples_batch(
        ...
        for index in range(len(examples["_prompt"])):
            try:
                ...
                lipread = (examples.get("_lipread") or [None] * len(examples["_prompt"]))[index] or []
                lipread_kwargs: dict = {"lipread": lipread} if lipread else {}
                ...
                try:
                    if self.data_args.build_sample_shape_index:
                        messages, layouts = self.template.mm_plugin.process_messages_with_layout(
                            raw_messages, images, videos, audios, self.processor, **lipread_kwargs
                        )
                    else:
                        messages = self.template.mm_plugin.process_messages(
                            raw_messages, images, videos, audios, self.processor, **lipread_kwargs
